...
//////// CONTROLS - MID
    private InputXY mTilt;
    private InputXY mDpad;
    private InputButton mJumpButton;
    private InputButton mAttackButton;
//////// CONTROLS - END
...
//////// CONTROLS - MID
// !!!! ???? TODO: OK? ???? !!!!
                if (mUseOrientationSensor && (mTilt != null)) {
                    parentObject.getTargetVelocity().x = 
                    	mTilt.getX() * mMovementSpeed;

                    parentObject.getTargetVelocity().y = 
                    	mTilt.getY() * mMovementSpeed;

                    parentObject.getAcceleration().x = mAcceleration;
                    parentObject.getAcceleration().y = mAcceleration; 
                } else if (mDpad != null) {
                    parentObject.getTargetVelocity().x = 
                    	mDpad.getX() * mMovementSpeed;
                ...
                if (mJumpButton != null) {
                    if (mJumpButton.getTriggered(gameTime) 
                            && parentObject.touchingGround()
                            && parentObject.getVelocity().y <= 0.0f
                            && !mChangeActionOnButton) {
                        parentObject.getImpulse().y += mJumpImpulse;
                    } else if (mChangeActionOnButton && mJumpButton.getPressed()) {
                        parentObject.setCurrentAction(mButtonPressedAction);
                    }
                }

                if ((mAttackButton != null) && mAttackButton.getTriggered(gameTime)) {
                    timeToRelease = true;
                }
//////// CONTROLS - END
            }
...
//////// CONTROLS - MID
    public void setTilt(InputXY tilt) {
        mTilt = tilt;
    }

    public void setDPad(InputXY dpad) {
        mDpad = dpad;
    }

    public void setJumpButton(InputButton jumpButton) {
        mJumpButton = jumpButton;
    }

    public void setAttackButton(InputButton attackButton) {
